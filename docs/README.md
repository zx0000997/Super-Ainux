# SAS Documentation / SAS 文档

| Language | Document |
|----------|----------|
| **中文** | [SAS_SYSTEM_SPEC.zh.md](./SAS_SYSTEM_SPEC.zh.md) — Super Ainux System 系统开发规格说明书 |
| **English** | [SAS_SYSTEM_SPEC.en.md](./SAS_SYSTEM_SPEC.en.md) — System Development Specification |
| **对话交接 / Session handoff** | [SAS_CONTINUATION_CONTEXT.zh.md](./SAS_CONTINUATION_CONTEXT.zh.md) — 跨设备续聊摘要 |

Kernel / OS baseline (Linux build checklist): see [`../OS_DEVELOPMENT_GUIDE.md`](../OS_DEVELOPMENT_GUIDE.md).

| Milestone | Document |
|-----------|----------|
| **M0** (baseline host, Compose, secrets) | [M0_DEPLOYMENT.zh.md](./M0_DEPLOYMENT.zh.md) — 部署与验收；实现目录 [`../m0/`](../m0/) |
| **M1** (task FSM, audit chain, stub adapters) | [M1_API.zh.md](./M1_API.zh.md) — API 说明；实现目录 [`../m1/`](../m1/) |
